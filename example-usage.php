...
error_log( 'api.wordpress.org query for woocommerce.' );
		...
		if ( $last_updated_age > (2 * WEEK_IN_SECONDS) ) {
			error_log( 'WooCommerce release is older than 2 weeks so can update.' );
			return $update;
		}
		else {
			error_log( 'WooCommerce release is less than two weeks old so do not update.' );
			global $ConditionalAutoUpdateBlocking;
